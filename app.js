...
// ═══════════════════════════════════════════════════════════════════════════
// CrewBoard: 크루 게시판 (Phase 7 — JSONBin 실시간 연동)
// ═══════════════════════════════════════════════════════════════════════════
const CrewBoard = (() => {
  const LOCAL_KEY = CONFIG.KEYS.BOARD_POSTS;
  let sortMode = 'latest';
  let _isSyncing = false; // 중복 요청 방지

  // ── JSONBin API ──────────────────────────────────────────────────────────
  const BIN_URL = CONFIG.JSONBIN.URL + CONFIG.JSONBIN.BIN_ID;
  const HEADERS = {
    'Content-Type': 'application/json',
    'X-Master-Key': CONFIG.JSONBIN.API_KEY,
    'X-Bin-Versioning': 'false', // 버전 관리 끄기 (단순 덮어쓰기)
  };
  ...
  async function _fetchPosts() {
    try {
      ...
      const posts = (Array.isArray(data.record) ? data.record : [])
        .filter(p => p.id && Array.isArray(p.likes)); // 유효한 글만 필터링
      _setLocal(posts);
      return posts;
    } catch (e) {
      console.warn('JSONBin fetch 실패, 로컬 캐시 사용:', e);
      return _getLocal();
    }
  }

  async function _savePosts(posts) {
    _setLocal(posts); // 낙관적 업데이트: 로컬 먼저 반영
    try {
      await fetch(BIN_URL, {
        method: 'PUT',
        headers: HEADERS,
      ...
      console.warn('JSONBin save 실패:', e);
      showToast('저장 중 오류가 발생했어요. 잠시 후 다시 시도해 주세요.', 'error');
    }
  }
  ...
  function render(postsOverride) {
    ...
    feed.innerHTML = posts.map(p => {
      ...
      const isMe = p.authorId === myId;

      const commentsHtml = p.comments.length ? `
        <div class="mt-3 space-y-1.5">
          ${p.comments.map(c => `
            <div class="flex items-start gap-2 bg-gray-50 rounded-xl px-3 py-2">
              <span class="text-base leading-none mt-0.5">${c.authorAvatar}</span>
              <div class="min-w-0">
                <span class="text-xs font-bold text-gray-700">${c.authorName}</span>
                <span class="text-xs text-gray-300 ml-1">${_relTime(c.createdAt)}</span>
                <p class="text-xs text-gray-600 break-all">${c.content}</p>
              </div>
            </div>`).join('')}
        </div>` : '';
      ...
      return `
        <div id="post-card-${p.id}" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <div class="flex gap-1.5 mt-3">
            <input id="${inputId}" type="text" maxlength="20" placeholder="응원 한마디... (20자)"
  ...
  // ── 최초 로드: JSONBin에서 받아와서 렌더링 ───────────────────────────────
  async function init() {
    render(_getLocal()); // 로컬 캐시로 즉시 렌더
    const posts = await _fetchPosts(); // JSONBin에서 최신 데이터
    render(posts);
  }

  // ── 30초마다 자동 폴링 (크루원 글 실시간 반영) ──────────────────────────
  function startPolling() {
    setInterval(async () => {
      if (document.getElementById('page-board')?.classList.contains('hidden')) return;
  ...
  return {
    ...
    setSortMode, deletePost, startEdit, saveEdit, cancelEdit, previewPractice,
  };
})();
// ═══════════════════════════════════════════════════════════════════════════
