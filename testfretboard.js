...
const {
  ...
  positionBoxes = null,
    ...
    // ── 포지션 배경 박스 ────────────────────────────────────────────
    if (positionBoxes) {
      positionBoxes.forEach(({ color, opacity, perString }) => {
        perString.forEach(({ s, lo, hi }) => {
          const cy = MARGIN_TOP + (STRINGS - 1 - s) * STRING_GAP;
          const y1 = cy - STRING_GAP / 2;
          const x1 = (lo === 0 && startFret === 0)
            ? MARGIN_LEFT - 20
            : MARGIN_LEFT + fretPos(lo - 1, startFret, endFret, fretAreaW);
          const x2 = MARGIN_LEFT + fretPos(hi, startFret, endFret, fretAreaW);
          if (x2 <= x1) return;
          svg.appendChild(el('rect', {
            x: x1, y: y1, width: x2 - x1, height: STRING_GAP,
